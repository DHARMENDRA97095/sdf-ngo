    ...
    <div className="bg-bg-color min-h-screen pb-20">
      ...
      <section className="bg-secondary text-white py-20 bg-opacity-90 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl max-w-2xl mx-auto text-green-50">Impact-driven initiatives targeting the most critical challenges facing our communities today.</p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {programsList.map((program, idx) => (
            <div key={idx} id={program.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group flex flex-col scroll-mt-28">
              ...
              <div className="p-8 relative grow flex flex-col">
                ...
                <h3 className="text-2xl font-serif font-bold text-text-primary mb-4 mt-4 leading-tight">{program.title}</h3>
                <p className="text-gray-600 mb-6 grow">{program.desc}</p>
                <div className="flex items-center gap-2 mt-auto pt-4 border-t border-gray-100">
                  <span className="bg-[#E9EFE1] text-primary text-xs font-bold px-3 py-1 rounded-full">{program.stats.beneficiaries} Beneficiaries</span>
                  <span className="bg-blue-50 text-accent text-xs font-bold px-3 py-1 rounded-full">{program.stats.regions} Active</span>
                </div>
