...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    $image_url = !empty(trim($_POST['image_url'])) ? trim($_POST['image_url']) : 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=400&q=80';
    ...
    if (!empty($title) && !empty($category) && !empty($location) && !empty($description)) {
        ...
        $stmt->execute([$title, $category, $location, $description, $image_url, $status]);
        header("location: projects.php");
        exit;
    }
}
...
<body class="bg-gray-50 font-sans flex min-h-screen">
    ...
    <main class="flex-1 flex flex-col overflow-hidden relative">
        ...
        <!-- Add Project Modal -->
        <div id="addProjectModal" class="fixed inset-0 bg-black/50 z-50 flex justify-center items-center hidden p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <form action="projects.php" method="POST" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="url" name="image_url" placeholder="https://unsplash.com/..." class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 outline-none">
                        <p class="text-xs text-gray-400 mt-1">Leave blank for default image.</p>
                    </div>
                    <div>
